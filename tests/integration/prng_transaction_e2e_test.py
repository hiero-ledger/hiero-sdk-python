...
def test_integration_prng_transaction_can_execute(env):
    ...
    assert record.prng_bytes is None, "PRNG bytes should be None when range is set"
...
def test_integration_prng_transaction_can_execute_without_range(env):
    ...
    assert record.prng_number is None, "PRNG number should be None when no range is set"
