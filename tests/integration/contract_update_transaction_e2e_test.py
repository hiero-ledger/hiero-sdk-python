...
@pytest.mark.integration 
def test_integration_contract_update_transaction_set_contract_id_method(env):
    """Test that set_contract_id method works correctly."""
    # Create file with smart contract bytecode
    file_receipt = (
        FileCreateTransaction()
        .set_keys(env.operator_key.public_key())
        .set_contents(STATEFUL_CONTRACT_BYTECODE)
        .set_file_memo("[e2e::ContractUpdateTransaction] set_contract_id test")
        .execute(env.client)
    )
    ...
    contract_receipt = (
        ...
        .set_constructor_parameters(params)
        .set_bytecode_file_id(file_id)
        .set_contract_memo("[e2e::ContractCreateTransaction] for set_contract_id test")
        .execute(env.client)
    )
    ...
    # Test method chaining with set_contract_id
    update_tx = ContractUpdateTransaction()
    returned_tx = update_tx.set_contract_id(contract_id)
    
    # Verify method chaining returns same instance
    assert returned_tx is update_tx, "set_contract_id should return self for method chaining"
