...
def test_setters_and_constructor(mock_account_ids):
    ...
    test_fee_list = [
        CustomFixedFee(amount=100, fee_collector=collector_account)
    ]
...
def test_build_raises_error_if_no_token_id():
    ...
    # 2. Set some fees
    test_fee_list = [
        CustomFixedFee(amount=100, fee_collector=AccountId(0, 0, 987))
    ]
...
def test_build_transaction_body_correctly(mock_account_ids):
    ...
    assert len(transaction_body.tokenFeeScheduleUpdate.customFees) == 1
    
    # Check the details of the fee itself
    proto_fee = transaction_body.tokenFeeScheduleUpdate.customFees[0]
