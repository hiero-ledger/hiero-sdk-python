...
# ---------------------------------------------------------------------------
# Additional coverage: fluent setters, edge cases, bool rejection
# ---------------------------------------------------------------------------


class TestFluentSetterReturnSelf:
    """Verify all fluent setters return the transaction instance."""

    def test_create_setters_return_self(self):
        tx = RegisteredNodeCreateTransaction()
        key = PrivateKey.generate_ed25519().public_key()
        assert tx.set_admin_key(key) is tx
        assert tx.set_description("desc") is tx
        assert tx.set_service_endpoints([_make_block_endpoint()]) is tx
        assert tx.add_service_endpoint(_make_mirror_endpoint()) is tx

    def test_update_setters_return_self(self):
        tx = RegisteredNodeUpdateTransaction()
        key = PrivateKey.generate_ed25519().public_key()
        assert tx.set_registered_node_id(1) is tx
        assert tx.set_admin_key(key) is tx
        assert tx.set_description("desc") is tx
        assert tx.set_service_endpoints([_make_block_endpoint()]) is tx
        assert tx.add_service_endpoint(_make_mirror_endpoint()) is tx

    def test_delete_setters_return_self(self):
        tx = RegisteredNodeDeleteTransaction()
        assert tx.set_registered_node_id(1) is tx


class TestCreateMissingAdminKey:
    def test_fails_without_admin_key(self, mock_account_ids):
        operator_id, _, node_account_id, _, _ = mock_account_ids
        tx = RegisteredNodeCreateTransaction()
        tx.set_service_endpoints([_make_block_endpoint()])
        tx.operator_account_id = operator_id
...
class TestUpdateNoServiceEndpointsRoundTrip:
    def test_round_trip_without_service_endpoints(self):
        tx = RegisteredNodeUpdateTransaction()
        tx.set_registered_node_id(5)
        tx.set_description("no endpoints")

        _freeze(tx)
        restored = Transaction.from_bytes(tx.to_bytes())

        assert isinstance(restored, RegisteredNodeUpdateTransaction)
        assert restored.registered_node_id == 5
        assert restored.description == "no endpoints"
        assert restored.service_endpoints is None


class TestBlockEndpointBoolRejection:
    def test_rejects_bool_in_endpoint_apis(self):
