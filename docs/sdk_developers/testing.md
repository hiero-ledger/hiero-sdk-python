...
**File:** `tests/unit/tokens/test_token_transfer.py`

```python
"""
Unit tests for token transfer functionality.
"""
import pytest
...
def test_token_transfer_initialization():
    """Test that TokenTransfer initializes correctly."""
    token_id = TokenId(0, 0, 12345)
    sender_id = AccountId(0, 0, 1001)
    receiver_id = AccountId(0, 0, 1002)
    amount = 100
...
def test_token_transfer_validates_positive_amount():
    """Test that TokenTransfer rejects negative amounts."""
    token_id = TokenId(0, 0, 12345)
    sender_id = AccountId(0, 0, 1001)
    receiver_id = AccountId(0, 0, 1002)
...
def test_token_transfer_serialization():
    """Test that TokenTransfer serializes correctly."""
    token_id = TokenId(0, 0, 12345)
    sender_id = AccountId(0, 0, 1001)
    receiver_id = AccountId(0, 0, 1002)
    amount = 100
    ...
    serialized = transfer.to_proto()
    
    assert serialized is not None
    # Add specific assertions about the protobuf structure
```

#### 2. Create Integration Test

**File:** `tests/integration/token_transfer_e2e_test.py`

```python
"""
Integration tests for token transfer functionality.
"""
import pytest
...
@pytest.mark.integration
def test_integration_token_transfer_fungible_token(env):
    """Test that fungible tokens can be transferred between accounts."""
    ...
    token_receipt = (
        ...
        .set_token_symbol("TST")
        .set_decimals(2)
        .set_initial_supply(1000)
        .set_treasury_account_id(sender.id)
        .set_admin_key(env.operator_key.public_key())
        .set_supply_key(env.operator_key.public_key())
    ...
    transfer_receipt = (
        TransferTransaction()
        .add_token_transfer(token_id, sender.id, -transfer_amount)
        .add_token_transfer(token_id, receiver.id, transfer_amount)
        .freeze_with(env.client)
        .sign(sender.key)
        .execute(env.client)
    )
    ...
    # Verify balances
    sender_balance = CryptoGetAccountBalanceQuery(sender.id).execute(env.client)
    receiver_balance = CryptoGetAccountBalanceQuery(receiver.id).execute(env.client)
    
    assert sender_balance.tokens[token_id] == 900  # 1000 - 100
    assert receiver_balance.tokens[token_id] == 100


@pytest.mark.integration
def test_integration_token_transfer_fails_without_association(env):
    """Test that token transfer fails if receiver hasn't associated the token."""
    sender = env.create_account()
    receiver = env.create_account()
    ...
    # Try to transfer without association
    transfer_receipt = (
        TransferTransaction()
        .add_token_transfer(token_id, sender.id, -100)
    ...
    assert transfer_receipt.status == ResponseCode.TOKEN_NOT_ASSOCIATED_TO_ACCOUNT
```
...
**File:** `tests/unit/test_account_id.py`

```python
"""
Unit tests for AccountId class.
"""
import pytest
from hiero_sdk_python.account.account_id import AccountId


def test_account_id_initialization():
    """Test AccountId can be initialized with shard, realm, num."""
...
def test_account_id_from_string():
    """Test AccountId can be parsed from string."""
...
def test_account_id_to_string():
    """Test AccountId converts to string correctly."""
    account_id = AccountId(0, 0, 1234)
    
    assert str(account_id) == "0.0.1234"


def test_account_id_equality():
    """Test AccountId equality comparison."""
    account_id1 = AccountId(0, 0, 1234)
    account_id2 = AccountId(0, 0, 1234)
    account_id3 = AccountId(0, 0, 5678)
...
def test_account_id_from_string_invalid(invalid_string):
    """Test AccountId rejects invalid string formats."""
    with pytest.raises(ValueError):
        AccountId.from_string(invalid_string)
```

### Simple Integration Test Example

**File:** `tests/integration/account_balance_query_e2e_test.py`

```python
"""
Integration tests for account balance queries.
"""
import pytest
from hiero_sdk_python.query.account_balance_query import CryptoGetAccountBalanceQuery
from hiero_sdk_python.hbar import Hbar
...
@pytest.mark.integration
def test_integration_account_balance_query_can_execute(env):
    """Test that account balance can be queried."""
    balance = CryptoGetAccountBalanceQuery(
        account_id=env.operator_id
    ).execute(env.client)
    
    assert balance is not None
    assert balance.hbars >= Hbar(0)


@pytest.mark.integration
def test_integration_new_account_has_correct_balance(env):
    """Test that newly created account has expected balance."""
    initial_balance = Hbar(2)
    account = env.create_account(initial_balance=initial_balance)
    
    balance = CryptoGetAccountBalanceQuery(
        account_id=account.id
    ).execute(env.client)
    
    assert balance.hbars.to_tinybars() == initial_balance.to_tinybars()


@pytest.mark.integration
def test_integration_balance_query_invalid_account(env):
    """Test that balance query fails for invalid account."""
    from hiero_sdk_python.account.account_id import AccountId
    from hiero_sdk_python.exceptions import PrecheckError
    
    invalid_account = AccountId(0, 0, 999999999)
