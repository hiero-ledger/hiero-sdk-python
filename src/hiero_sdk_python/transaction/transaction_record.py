...
class TransactionRecord:
    ...
    Attributes:
        ...
        consensus_timestamp: Optional[Timestamp] = None:  The consensus timestamp of the transaction.
        schedule_ref: Optional[ScheduleId] = None:        Reference to the schedule that triggered this transaction (if scheduled).
        assessed_custom_fees: list[AssessedCustomFee] = field(default_factory=list):   
                                                          Custom fees assessed during the transaction.
        automatic_token_associations: list[TokenAssociation] = field(default_factory=list):
                                                          Token associations automatically created during the transaction.
        parent_consensus_timestamp: Optional[Timestamp] = None: 
                                                          The consensus timestamp of the parent transaction (for child transactions).
        alias: Optional[bytes] = None:                    The alias of a newly created account (if applicable).
        ethereum_hash: Optional[bytes] = None:            The Ethereum hash for EVM-compatible transactions.
        paid_staking_rewards: list[tuple[AccountId, int]] = field(default_factory=list):
                                                          Staking rewards paid out during this transaction.
        evm_address: Optional[bytes] = None:              The EVM address derived from the account.
        contract_create_result: Optional[ContractFunctionResult] = None:
                                                          The result of a contract creation if applicable.
    """
    ...
    def __repr__(self) -> str:
        ...
        if self.receipt:
            try:
                ...
                if status and status.startswith("UNKNOWN_CODE_"):
