...
def print_fractional_fees(token_info):
    ...
    fractional_fees = [f for f in token_info.custom_fees if isinstance(f, CustomFractionalFee)]
    if not fractional_fees:
        print("No fractional fees found.")
        return

    for i, fee in enumerate(fractional_fees, start=1):
        print(f"Fractional Fee #{i}:")
        ...
        print(f"  Assessment Method       = {fee.assessment_method.name}")
        print(f"  Fee Collector Account   = {fee.fee_collector_account_id}")
