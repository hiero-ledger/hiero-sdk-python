...
def airdrop_tokens(client, operator_id, operator_key, recipient_id, token_ids):
    ...
    try:
        ...
        for t in token_ids:
            ...
            print(
                f" {str(t)}: sender={sender_balances_before.get(t, 0)} recipient={recipient_balances_before.get(t, 0)}"
            )
        ...
        receipt = tx.freeze_with(client).sign(operator_key).execute(client)
        print(
            f"Token airdrop executed: status={receipt.status} transaction_id={receipt.transaction_id}"
        )
        ...
        for t in token_ids:
            # token_ids elements are TokenId objects (not strings), so use them as dict keys
            print(
                f" {str(t)}: sender={sender_balances_after.get(t, 0)} recipient={recipient_balances_after.get(t, 0)}"
            )
